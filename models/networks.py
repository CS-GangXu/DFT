...
def define_G(opt):
    ...
    if which_model == 'ConditionNet':
        ...
        import models.modules.baseline as net
        netG = net.SRResNet(in_nc=opt_net['in_nc'], out_nc=opt_net['out_nc'], nf=opt_net['nf'], nb=opt_net['nb'], act_type=opt_net['act_type'], opt=opt_net)
    elif which_model == 'DFT':
        import models.modules.network as net
        netG = net.DFT(opt=opt_net)
    else:
        try:
            sys.path.append('models/modules')
            imported_module = __import__(which_model)
            netG = imported_module.Restormer(opt=opt_net)
        except:
